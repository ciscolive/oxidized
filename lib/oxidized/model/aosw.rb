class AOSW < Oxidized::Model
  ...
  cmd "show inventory" do |cfg|
    cfg = "" if /(Invalid input detected at '\^' marker|Parse error)/.match?(cfg) # Don't show for unsupported devices (IAP and MAS)
  ...
  cmd "show running-config" do |cfg|
    ...
    cfg.each_line do |line|
      next if /^controller config \d+$/.match?(line)
      next if /^Building Configuration/.match?(line)
  ...
  def clean(cfg)
    ...
    cfg.each_line do |line|
      ...
      next if /Output \d Config/i.match?(line)
      next if /(Tachometers|Temperatures|Voltages)/.match?(line)
      next if /((Card|CPU) Temperature|Chassis Fan|VMON1[0-9])/.match?(line)
      next if /[0-9]+\s+(RPMS?|m?V|C)/i.match?(line)
