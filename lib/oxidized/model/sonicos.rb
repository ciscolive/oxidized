class SonicOS < Oxidized::Model
  ...
  def clean(cfg)
    ...
    cfg.each_line do |line|
      next if /date \d{4}:\d{2}:\d{2}/.match?(line)
      next if /time \d{2}:\d{2}:\d{2}/.match?(line)
      next if /system-time "\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}:\d{2}.\d+"/.match?(line)
      next if /system-uptime "((\s+up\s+\d+\s+)|(\d+\s\w+(,\s)?)*)"/.match?(line)
      next if /checksum \d+/.match?(line)
