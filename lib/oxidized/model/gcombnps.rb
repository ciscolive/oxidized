class GcomBNPS < Oxidized::Model
  ...
  cmd "show interface sfp" do |cfg|
    ...
    cfg.each_line do |line|
      next if /^  Temperature/.match?(line)
      next if /^  Voltage\(V\)/.match?(line)
      next if /^  Bias Current\(mA\)/.match?(line)
      next if /^  RX Power\(dBM\)/.match?(line)
      next if /^  TX Power\(dBM\)/.match?(line)
  ...
  cmd "show system" do |cfg|
    out = []
    cfg.each_line do |line|
      next if /^system run time        :/.match?(line)
      next if /^switch temperature     :/.match?(line)
